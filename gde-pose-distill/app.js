...
async function init() {
    updateStatus('載入 GDE Pose Distill 模型中...');
    ...
    try {
        ...
        model = await tf.loadGraphModel('../models/gde-pose-distill/model.json', {
            onProgress: (fraction) => {
...
async function startRealtimeDetection() {
    ...
    try {
        ...
        inputCtx.fillRect(20, 20, 50, 50);
        
        outputCtx.fillStyle = 'red';
        outputCtx.fillRect(100, 100, 80, 80);
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // 確保輸入和輸出畫布有足夠的視覺區分
    inputCanvas.style.border = '1px solid blue';
    outputCanvas.style.border = '1px solid red';
    
    // 調整容器樣式
    const inputContainer = document.querySelector('.image-container:first-child');
    const outputContainer = document.querySelector('.image-container:last-child');
    ...
    if (inputContainer && outputContainer) {
        ...
        outputContainer.style.background = '#f8f8f8';
    } else {
        console.warn("未找到畫布容器");
    }
    ...
    setTimeout(() => {
        ...
        outputCtx.beginPath();
        outputCtx.moveTo(150, 150);
        outputCtx.lineTo(300, 300);
        outputCtx.stroke();
        
        outputCtx.fillStyle = 'yellow';
        outputCtx.beginPath();
        outputCtx.arc(400, 200, 50, 0, Math.PI * 2);
        outputCtx.fill();
        
        console.log("在輸出畫布上繪製了顯眼的測試圖形");
    }, 1000);
});
