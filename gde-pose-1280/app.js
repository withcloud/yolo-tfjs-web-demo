...
// Default canvas size - 使用1280x1280
...
async function init() {
    updateStatus('載入 GDE Pose (1280) 模型中...');
    ...
    try {
        ...
        // Load the model - 使用GDE模型路徑
        updateLoadingStatus('正在載入模型文件...');
        model = await tf.loadGraphModel('../models/gde-pose-1280/model.json', {
            onProgress: (fraction) => {
