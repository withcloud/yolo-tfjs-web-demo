...
async function init() {
    ...
    try {
        ...
        // Load the model - 使用GDE Yoga模型路徑
        updateLoadingStatus('正在載入模型文件...');
        model = await tf.loadGraphModel('../models/gde-yoga-640/model.json', {
            onProgress: (fraction) => {
...
async function processImage(img) {
    ...
    try {
        ...
        updateStatus('檢測完成！');
        updateInfo(`在圖中檢測到 ${predictions.length} 個瑜伽姿勢。`);
    } catch (error) {
...
async function runDetection(video) {
    try {
        ...
        console.log("檢測完成，找到", predictions.length, "個瑜伽姿勢");
        
        // 更新檢測信息
        updateInfo(`檢測到 ${predictions.length} 個瑜伽姿勢`);
