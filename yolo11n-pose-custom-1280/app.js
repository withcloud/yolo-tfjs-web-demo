...
// Default canvas size - 注意這裡使用1280x1280而不是640x640
...
async function init() {
    updateStatus('載入 YOLO Pose (Custom 1280) 模型中...');
    ...
    try {
        ...
        // Load the model - 注意這裡使用不同的模型路徑
        updateLoadingStatus('正在載入模型文件...');
        model = await tf.loadGraphModel('../models/yolo11n-pose-custom-1280/model.json', {
            onProgress: (fraction) => {
        ...
        // Warm up the model - 注意這裡使用1280x1280
        const dummyInput = tf.zeros([1, 1280, 1280, 3]);
    ...
    try {
        ...
        const tensorData = tf.tidy(() => {
            ...
            if (batchedShape.length !== 4 || batchedShape[0] !== 1 || 
                batchedShape[1] !== 1280 || batchedShape[2] !== 1280 || batchedShape[3] !== 3) {
                console.error('批處理後的張量形狀不正確:', batchedShape);
                console.error('預期形狀: [1, 1280, 1280, 3]');
                return null;
            }
