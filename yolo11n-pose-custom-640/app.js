...
async function init() {
    updateStatus('載入 YOLOv11n Pose 模型中...');
    ...
    try {
        ...
        // Load the model - 使用YOLOv11n Pose模型路徑
        updateLoadingStatus('正在載入模型文件...');
        model = await tf.loadGraphModel('../models/yolo11n-pose-custom-640/model.json', {
            onProgress: (fraction) => {
...
async function processImage(img) {
    ...
    try {
        ...
        updateStatus('檢測完成！');
        updateInfo(`在圖中檢測到 ${predictions.length} 個姿勢。`);
    } catch (error) {
...
async function runDetection(video) {
    try {
        ...
        console.log("檢測完成，找到", predictions.length, "個姿勢");
        
        // 更新檢測信息
        updateInfo(`檢測到 ${predictions.length} 個姿勢`);
